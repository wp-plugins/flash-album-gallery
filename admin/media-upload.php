...
function media_upload_flag_form($errors) {
	...
	$form_action_url = get_option('siteurl') . "/wp-admin/media-upload.php?type={$GLOBALS['type']}&tab=flag&post_id=$post_id";
	...
	// Get the images
	if ( $galleryID != 0 )
		$picarray = $wpdb->get_col("SELECT pid FROM $wpdb->flagpictures WHERE galleryid = '$galleryID' ORDER BY {$flag->options['galSort']} {$flag->options['galSortDir']} LIMIT $start, 10 ");	
...
<div class="tablenav">
	...
	<div class="alignleft actions">
		<select id="select_gal" name="select_gal" style="width:200px;">;
			<option value="0" <?php selected('0', $galleryID); ?> ><?php attribute_escape( _e('No gallery',"flag") ); ?></option>
			<?php
		...
		<input type="submit" id="show-gallery" value="<?php attribute_escape( _e('Select &#187;','flag') ); ?>" class="button-secondary" />
	</div>
	<br style="clear:both;" />
</div>
</form>
...
<form enctype="multipart/form-data" method="post" action="<?php echo attribute_escape($form_action_url); ?>" class="media-upload-form" id="library-form">
	...
	if(is_array($picarray)) {
		foreach ($picarray as $picid) {
			...
			<div id='media-item-<?php echo $picid ?>' class='media-item preloaded'>
			  ...
			  <a class='toggle describe-toggle-on' href='#'><?php attribute_escape( _e('Show', "flag") ); ?></a>
			  <a class='toggle describe-toggle-off' href='#'><?php attribute_escape( _e('Hide', "flag") );?></a>
			  ...
			  <table class='slidetoggle describe startclosed'><tbody>
				  <tr>
					...
					<td><?php attribute_escape( _e('Image ID:', "flag") ); ?><?php echo $picid ?></td>
				  </tr>
				  <tr><td><?php echo attribute_escape( $picture->filename ); ?></td></tr>
				  <tr><td><?php echo attribute_escape( stripslashes($picture->alttext) ); ?></td></tr>
				  <tr><td>&nbsp;</td></tr>
				  <tr>
					<td class="label"><label for="image[<?php echo $picid ?>][alttext]"><?php attribute_escape( _e('Alt/Title text', "flag") );?></label></td>
				  ...
				  <tr>
					<td class="label"><label for="image[<?php echo $picid ?>][description]"><?php attribute_escape( _e("Description","flag") ); ?></label></td>
	...
	<p class="ml-submit">
		<input type="submit" class="button savebutton" name="save" value="<?php attribute_escape( _e('Save all changes','flag') ); ?>" />
	</p>
