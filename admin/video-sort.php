...
if(count($items_a)) {
	...
	foreach($items_a as $item) {
		...
		<tr id="$flv-<?php echo $flv->ID; ?>" class="<?php echo $alternate; ?> iedit"  valign="top">
				<td scope="row"><strong><?php echo $flv->ID; ?></strong></td>
				<td><a class="thickbox" href="<?php echo $flv->guid; ?>"><img id="thumb-<?php echo $flv->ID; ?>" src="<?php echo $thumb; ?>" width="20" height="20" alt="" /></a></td>
				<td><?php echo basename($flv->guid); ?></td>
				<td><input type="hidden" name="item_a[<?php echo $flv->ID; ?>][post_title]" value="<?php echo $flv->post_title; ?>" /></td>
		</tr>
		<?php
	}
} else {
