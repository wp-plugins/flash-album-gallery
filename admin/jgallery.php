	...
	<?php if (is_array ($thepictures) && count($thepictures)){ ?>
		...
		<div class="flagcategory" id="gid_<?php echo $galID.'_'.$skinID; ?>">
			<?php $n = count($thepictures);
				...
				$j=0;
		if ($isCrawler){
			foreach ($thepictures as $picture) { ?><a style="display:block; overflow: hidden; height: auto; width: auto; margin-bottom: 10px; background-color: #eeeeee; background-position: 22px 44px; text-align: left;" class="i<?php echo $j++; ?>" href="<?php echo $siteurl.'/'.$picture->path.'/'.$picture->filename; ?>" id="flag_pic_<?php echo $picture->pid; ?>" rel="gid_<?php echo $galID.'_'.$skinID; ?>"><img style="float:left; margin-right: 10px;" title="<?php echo strip_tags(stripslashes($picture->alttext)); ?>" alt="<?php echo strip_tags(stripslashes($picture->alttext)); ?>" src="<?php echo $siteurl.'/'.$picture->path.'/thumbs/thumbs_'.$picture->filename; ?>" width="115" height="100" /><span style="display: block; overflow: hidden; text-decoration: none; color: #000; font-weight: normal;" id="flag_desc_<?php echo $picture->pid; ?>"><strong><?php echo strip_tags(stripslashes($picture->alttext)); ?></strong><br /><?php echo stripslashes($picture->description); ?></span></a><?php
			}
		} else {
