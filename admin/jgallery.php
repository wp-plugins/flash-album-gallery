...
?>
	<?php if (is_array ($thepictures) && count($thepictures)){ ?>
		...
		if ($isCrawler){
			...
			foreach ($thepictures as $picture) { ?><a class="i<?php echo $j++; ?> flag_pic_alt" href="<?php echo $siteurl.'/'.$picture->path.'/'.$picture->filename; ?>" id="flag_pic_<?php echo $picture->pid; ?>" rel="gid_<?php echo $galID.'_'.$skinID; ?>" title="<?php echo strip_tags(stripslashes($picture->alttext)); ?>">[img alt="" src="<?php echo $siteurl.'/'.$picture->path.'/thumbs/thumbs_'.$picture->filename; ?>"]<span class="flag_pic_desc" id="flag_desc_<?php echo $picture->pid; ?>"><strong><?php echo htmlspecialchars(stripslashes($picture->alttext)); ?></strong><br /><span><?php echo htmlspecialchars(stripslashes($picture->description)); ?></span></span></a><?php
			}
		} ?>
		</div>
	<?php } ?>
